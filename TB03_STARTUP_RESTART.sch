...
E {}
B 4 420 240 420 260 {}
T {TB03_STARTUP_RESTART: Startup and restart} 220 50 0 0 0.5 0.5 {}
T {Startup/restart: all 135 model combinations, 3 temperatures, 7 supplies (2835 cases).} 220 90 0 0 0.27 0.27 {}
...
T {MODELS: bootstrap TT models and MIM compatibility model.
TEST: starts only this bench's corner sweep using Python 3 + ngspice.
Keep run_bandgap.py, bandgap_config.json and pvt_controls.json alongside this file.} 220 610 0 0 0.27 0.27 {}
T {Edit bandgap_config.json for sweep lists, timeouts, parallel jobs and output directory.
All corner combinations: MOS x BJT x resistor x MIM. Temperature: -40 / 25 / 125 C.
PSRR/startup VDD: 3.0 / 3.3 / 3.6 / 4.0 / 4.5 / 5.0 / 5.5 V.
Line: 3.0 to 5.5 V, step 10 mV; nominal 3.3 to 5 V reported separately.
Each test has its own text report and resumable per-case results.} 220 700 0 0 0.27 0.27 {}
...
C {vsource.sym} 900 240 0 0 {name=VDD value="PWL(0 0 1u 0 101u \{VDD_RUN\} 4m \{VDD_RUN\} 4001u 0 4500u 0 4501u \{VDD_RUN\} 6m \{VDD_RUN\})" savecurrent=false}
...
set noaskquit
echo BANDGAP_SINGLE_RUNNER_V9_TB03_STARTUP_RESTART
shell python3 "/foss/designs/chipalooza-bandgap/Jonah_Saunders_Bandgap/run_bandgap.py" --deck "/headless/.xschem/simulations/TB03_STARTUP_RESTART.spice" --test TB03_STARTUP_RESTART
echo RUNNER_RETURNED_CHECK_REPORT_AND_ERRORS
.endc
"}
C {chipalooza-bandgap/Jonah_Saunders_Bandgap/Bandgap_Core.sym} 400 240 0 0 {name=x1}
